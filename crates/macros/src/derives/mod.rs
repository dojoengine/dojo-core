...
use crate::token_stream_ext::TokenStreamsExt;
...
fn handle_derives_macros(token_stream: TokenStream) -> ProcMacroResult {
...
pub fn dojo_derive_all(
...
) -> (TokenStream, Vec<Diagnostic>) {
    if attrs.is_empty() {
        return (TokenStream::empty(), vec![]);
    }
    ...
    let code = rewrite_nodes.join_to_token_stream("");

    let item_name = item_ast
        .as_syntax_node()
