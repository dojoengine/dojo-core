...
pub fn handle_print_struct(db: &dyn SyntaxGroup, struct_ast: ItemStruct) -> TokenStream {
...
",
        &HashMap::from([
            (
                "type_name".to_string(),
                struct_ast.name(db).as_syntax_node().get_text(db),
            ),
            ("print".to_string(), prints.join("\n")),
        ]),
    )
}
...
/// Returns:
/// * A TokenStream containing the generated code.
pub fn handle_print_enum(db: &dyn SyntaxGroup, enum_ast: ItemEnum) -> TokenStream {
...
",
        &HashMap::from([
            (
                "type_name".to_string(),
                enum_ast.name(db).as_syntax_node().get_text(db),
            ),
            ("print".to_string(), prints.join(",\n")),
        ]),
    )
