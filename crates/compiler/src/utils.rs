...
pub fn get_namespace_config(db: &dyn SyntaxGroup) -> Result<NamespaceConfig> {
    ...
    let configuration = match db.crate_config(
        *crates
            .first()
            .expect("No root crate found in the workspace."),
    ) {
        Option::Some(cfg) => cfg,
        Option::None => {
            return Err(anyhow::anyhow!(
                "No configuration found for the root crate."
            ))
        }
    };
