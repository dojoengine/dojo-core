...
mod tests {
    ...
    fn test_namespace_config_validate() {
        ...
        let empty_default_config = NamespaceConfig {
            default: "".to_string(),
            mappings: None,
        };
        assert!(empty_default_config.validate().is_err());

        let invalid_default_config = NamespaceConfig {
            default: "invalid-default".to_string(),
            mappings: None,
        };
    ...
    fn test_namespace_config_from_cfg_set() {
        ...
        cfg_set.insert(Cfg::kv(
            ...
            SmolStr::from("default_namespace"),
        ));
        cfg_set.insert(Cfg::kv(
            format!("{}tag1", NAMESPACE_CFG_PREFIX),
            SmolStr::from("namespace1"),
        ));
        cfg_set.insert(Cfg::kv(
            format!("{}tag2", NAMESPACE_CFG_PREFIX),
            SmolStr::from("namespace2"),
        ));
