...
pub fn compile_workspace(
    ...
    for unit in compilation_units {
        ...
        if let CompilationUnit::Cairo(mut unit) = unit {
            ...
            ui.verbose(format!(
                "target kind: {}",
                unit.main_component().target_kind()
            ));

            let compiler_config: CompilerConfig =
                DojoConfigLoader::from_package(&unit.components[0].package, &ws).unwrap();

            // let root_package_data =
            // PackageData::from_scarb_package(&unit.components[0].package)?;

            ui.verbose(compiler_config.namespace.display_mappings());
    ...
    let crate_roots = unit
        ...
        .map(|model| {
            (
                ...
                model.targets[0].source_root().into(),
            )
        })
        .collect();
...
pub fn cfg_set_from_component(
    ...
    let package_compiler_config: CompilerConfig = DojoConfigLoader::from_package(&c.package, ws)?;
    ...
    cfg_set.insert(Cfg {
        ...
        value: Some(package_compiler_config.namespace.default.into()),
    });

    cfg_set.insert(Cfg {
        key: WORKSPACE_CURRENT_PROFILE_CFG_KEY.into(),
        value: Some(
            ws.current_profile()
