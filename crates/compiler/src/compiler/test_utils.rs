...
pub fn copy_tmp_dir(source_dir: &Utf8PathBuf) -> Utf8PathBuf {
    ...
    copy_dir_recursively(
        ...
        &temp_project_dir.to_path_buf().into(),
    )
...
pub fn copy_tmp_config(
    ...
    let temp_project_path = temp_project_dir
        .join("Scarb")
        .with_extension("toml")
        .to_string();
...
pub fn copy_build_project_temp(
    ...
    let source_project_dir = Utf8PathBuf::from(source_project_path)
        .parent()
        .unwrap()
        .to_path_buf();
        ...
        if path.is_dir() {
            ...
            if file_name == "Scarb.toml" {
                ...
                let mut table = contents
                    .parse::<Table>()
                    .expect("Failed to parse Scab.toml");

                let dojo = if table.contains_key("workspace") {
                    table["workspace"]["dependencies"]["dojo"]
                        .as_table_mut()
                        .unwrap()
                } else {
                ...
                if dojo.contains_key("path") {
                    dojo["path"] = Value::String(
                        fs::canonicalize(dojo_core_path)
                            .unwrap()
                            .to_string_lossy()
                            .to_string(),
                    );
...
pub fn corelib() -> PathBuf {
    let config = build_test_config(
        "./src/manifest_test_data/spawn-and-move/Scarb.toml",
        Profile::DEV,
    )
    .unwrap();
